...
fun questTypeRegistry(
    ...
    */
    ...
    5 to AddBusStopName(), // requires text input
    6 to AddBusStopRef(), // requires text input
    // 7 to AddBusStopLit(), // at least during day requires to stand in it to see if there is a light in the shelter - moved
    ...
    // 28 to AddPowerPolesMaterial(), disable as unwanted
    ...
    *  for the sign which is one reason why it is disabled by default */
    ...
    // 152 to AddRoofShape(countryInfos, countryBoundariesFuture), removed as waste of time

    // 153 to AddStepCount(), // can only be gathered when walking along this way, also needs the most effort and least useful removed as waste of time
