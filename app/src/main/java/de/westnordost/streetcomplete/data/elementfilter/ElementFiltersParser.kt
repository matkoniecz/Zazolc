...
private fun StringWithCursor.parseElementDeclaration(): ElementsTypeFilter {
    ...
    return ELEMENT_TYPE_FILTERS_BY_NAME[word]
        ?: throw ParseException("Expected element types. Any of: nodes, ways or relations, separated by ','", cursorPos - word.length, string)
}
...
private fun StringWithCursor.parseElementFilter(): ElementFilter {
    ...
    if (nextIsAndAdvance(LIKE)) {
        ...
        throw ParseException("Unexpected operator '$operator': The key prefix operator '$LIKE' must be used together with the binary operator '$LIKE' or '$NOT_LIKE'", cursorPos, string)
    }
...
private fun StringWithCursor.parseTag(): String {
    ...
    if (word in RESERVED_WORDS) {
        throw ParseException("A key or value cannot be named like the reserved word '$word', surround it with quotation marks", cursorPos, string)
    }
    return word
}
