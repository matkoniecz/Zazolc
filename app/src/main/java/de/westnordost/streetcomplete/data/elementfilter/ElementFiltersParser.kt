...
private fun StringWithCursor.parseElementsDeclaration(): Set<ElementsTypeFilter> {
    ...
    do {
        ...
        if (result.contains(element)) {
            throw ParseException("Mentioned the same element type $element twice", cursor, string)
        }
        result.add(element)
...
private fun StringWithCursor.parseElementDeclaration(): ElementsTypeFilter {
    ...
    return ELEMENT_TYPE_FILTERS_BY_NAME[word]
        ?: throw ParseException("Expected element types. Any of: nodes, ways or relations, separated by ','", cursor - word.length, string)
}
...
private fun StringWithCursor.parseElementFilters(): BooleanExpression<ElementFilter, Element>? {
    ...
    if (!nextIsAndAdvance(WITH)) {
        if (!isAtEnd()) {
            throw ParseException("Expected end of string or '$WITH' keyword", cursor, string)
        }
        return null
    }
    ...
    do {
        ...
        if (!parseBracketsAndSpaces('(', builder)) {
            throw ParseException("Expected a whitespace or bracket before the tag", cursor, string)
        }
        ...
        if (!separated) {
            throw ParseException("Expected a whitespace or bracket after the tag", cursor, string)
        }
        ...
        if (nextIsAndAdvance(OR)) {
            ...
            throw ParseException("Expected end of string, '$AND' or '$OR'", cursor, string)
        }
    } while (true)
    ...
    try {
        ...
        throw ParseException(e.message, cursor, string)
    }
}
...
private fun StringWithCursor.parseElementFilter(): ElementFilter {
    ...
    if (nextIsAndAdvance(LIKE)) {
        ...
        throw ParseException("Unexpected operator '$operator': The key prefix operator '$LIKE' must be used together with the binary operator '$LIKE' or '$NOT_LIKE'", cursor, string)
    }
    ...
    if (operator in COMPARISON_OPERATORS) {
        ...
        throw ParseException("must either be a number (with optional unit) or a (relative) date", cursor, string)
    }
    throw ParseException("Unknown operator '$operator'", cursor, string)
}
...
private fun StringWithCursor.parseTag(): String {
    ...
    if (word in RESERVED_WORDS) {
        throw ParseException("A key or value cannot be named like the reserved word '$word', surround it with quotation marks", cursor, string)
    }
    return word
}
...
private fun StringWithCursor.parseQuotedWord(quot: Char): String? {
    ...
    while (true) {
        ...
        if (isAtEnd(length)) {
            throw ParseException("Did not close quotation marks", cursor - 1, string)
        }
        // ignore escaped
...
private fun StringWithCursor.parseWord(): String {
    ...
    if (length == 0) {
        throw ParseException("Missing value (dangling operator)", cursor, string)
    }
...
private fun StringWithCursor.parseNumber(): Float {
    ...
    try {
        ...
        throw ParseException("Expected a number", cursor, string)
    }
}
...
private fun StringWithCursor.parseDateFilter(): DateFilter {
    ...
    throw ParseException("Expected either a date (YYYY-MM-DD) or '$TODAY'", cursor, string)
}
...
private fun StringWithCursor.parseDurationInDays(): Float {
    ...
    return when {
        ...
        else -> throw ParseException("Expected $YEARS, $MONTHS, $WEEKS or $DAYS", cursor, string)
    }
}
...
private fun StringWithCursor.expectOneOrMoreSpaces(): Int {
    ...
    if (spaces == 0) {
        throw ParseException("Expected a whitespace", cursor, string)
    }
    return spaces
}
